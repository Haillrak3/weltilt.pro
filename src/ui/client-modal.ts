...
function renderEditTab(draft: DbClient, addAddrVisible: boolean): string {
  ...
  const af = (id: string, label: string) =>
    `<label class="client-field">
      <span>${escapeHtml(label)}</span>
      <input type="text" id="cm-raaf-${id}" class="client-input" value="" />
    </label>`;
  ...
  const addFormHtml = addAddrVisible
    ? `<div class="refs-add-addr-form">
        ...
        ${af('entrance', 'Подъезд')}
        ${af('floor', 'Этаж')}
        ${af('apartment', 'Квартира')}
        ${af('intercom', 'Код домофона')}
  ...
  function rerender(): void {
    ...
    overlay.innerHTML = `
      <div class="modal modal-wide cm-modal" role="dialog">
        ...
        <div class="cm-body">
          ${tab === 'edit' ? renderEditTab(draft, addAddrVisible) : renderOrdersTab(draft.phone)}
        </div>
